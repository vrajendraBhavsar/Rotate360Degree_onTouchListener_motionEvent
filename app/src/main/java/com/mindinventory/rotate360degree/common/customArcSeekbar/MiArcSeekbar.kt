...
class MiArcSeekbar @JvmOverloads constructor(
    context: Context,
    ...
    var maxProgress = a.useOrDefault(100) { getInteger(R.styleable.ArcSeekBar_maxProgress, it) }
        set(progress) {
            ...
            drawData?.let {
                drawData = it.copy(maxProgress = progress)
            }
            invalidate()
        }
...
/*    var progressWidth: Float = a.useOrDefault(4 * context.resources.displayMetrics.density) { getDimension(R.styleable.ArcSeekBar_progressWidth, it) }
    ...
    private var drawerDataObservers: List<(MiArcSeekBarData) -> Unit> = emptyList()

    private fun doWhenDrawerDataAreReady(f: (MiArcSeekBarData) -> Unit) {
        if (drawData != null) f(drawData!!) else drawerDataObservers += f
    }

    private var drawData: MiArcSeekBarData? = null
        set(value) {
            field = value ?: return
    ...
    private fun MiArcSeekBarData.drawThumb(canvas: Canvas) {
